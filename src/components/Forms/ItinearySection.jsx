...
//       let all = [];
//       if (destinations?.length && Array.isArray(destinations)) {
//         setIsLoading(true);
//         // Only fetch if destinations is a valid non-empty array
...
//     fetchAll();
//   }, [JSON.stringify(destinations)]); // Stringify to prevent unnecessary re-renders
...
const ItinerarySection = () => {
  ...
  const handleActivitySelect = (activity, index) => {
    ...
    setValue(`Itinearies.${index}.ImageUrl`, activity.ImageUrl || "");
    setValue(`Itinearies.${index}.Description`, "");
  };
